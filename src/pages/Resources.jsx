  ...
  return (
    <div className="page-shell">
      ...
      <div style={{ background: 'var(--gc-cream)', padding: '60px 44px 52px', textAlign: 'center' }}>
        <div style={{ font: '600 16px var(--font-sans)', letterSpacing: '.24em', textTransform: 'uppercase', color: 'var(--gc-emerald)', marginBottom: 16 }}>Resources</div>
        ...
        <p style={{ fontSize: 16, fontWeight: 300, color: 'var(--gc-ink-muted)', lineHeight: 1.85, maxWidth: 560, margin: '18px auto 0' }}>
      ...
      <div style={{ background: '#fff', padding: '54px 44px' }}>
        <div style={{ font: '700 16px var(--font-sans)', letterSpacing: '.2em', textTransform: 'uppercase', color: 'var(--gc-emerald)', marginBottom: 10, textAlign: 'center' }}>Cornerstone articles</div>
        ...
        <p style={{ fontSize: 16, fontWeight: 300, color: 'var(--gc-ink-muted)', lineHeight: 1.7, maxWidth: 520, margin: '0 auto 30px', textAlign: 'center' }}>
        ...
        {filter === 'All' ? (
          <div className="rgrid" style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 14, maxWidth: 900, margin: '0 auto' }}>
            {CORNERSTONE_ARTICLES.map((title) => (
              <div key={title} style={{ border: '1px solid var(--gc-border)', borderRadius: 6, padding: '20px 22px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
                <div style={{ fontFamily: 'var(--font-serif)', fontSize: 16, fontWeight: 700, color: 'var(--gc-slate)', lineHeight: 1.35 }}>{title}</div>
                <span style={{ flexShrink: 0, font: '700 16px var(--font-sans)', letterSpacing: '.12em', textTransform: 'uppercase', color: 'var(--gc-ink-muted)', background: 'var(--gc-section)', padding: '4px 10px', borderRadius: 100 }}>
                  Coming soon
                </span>
              </div>
            ))}
          ...
          <p style={{ fontSize: 16, fontWeight: 300, color: 'var(--gc-ink-muted)', textAlign: 'center', maxWidth: 420, margin: '0 auto' }}>
      ...
      <div style={{ background: 'var(--gc-navy-deep)', padding: '56px 44px' }}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          <div style={{ font: '700 16px var(--font-sans)', letterSpacing: '.22em', textTransform: 'uppercase', color: 'var(--gc-lavender-soft)', marginBottom: 12 }}>Free tools</div>
        ...
        <div className="rgrid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, maxWidth: 900, margin: '0 auto' }}>
          {TOOLS.map((t) => (
            <Link key={t.title} className="hover-lift" to={t.href} style={{ textDecoration: 'none', background: 'rgba(255,255,255,.06)', border: '1px solid rgba(255,255,255,.14)', borderRadius: 8, padding: '26px 24px', display: 'block' }}>
              ...
              <span style={{ font: '600 16px var(--font-sans)', letterSpacing: '.1em', textTransform: 'uppercase', color: t.accent }}>{t.cta}</span>
            </Link>
          ))}
        </div>
      </div>
      ...
      <div style={{ background: 'var(--gc-sage-light)', borderTop: '1px solid #dde6e2', padding: '52px 44px', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 16, fontWeight: 300, color: 'var(--gc-ink-muted)', lineHeight: 1.7, maxWidth: 460, margin: '0 auto 22px' }}>
